...
| 항목 | 기술 | 비고 |
|------|------|------|
| 얼굴 인식 | face-api.js (`@vladmandic/face-api`) | SSD MobileNet V1 |
...
| 이모티콘 | 얼굴 영역에 😊 이모티콘 오버레이 | 처리된 이미지로 업로드 목록 교체 |
...
### MediaPipe → face-api.js (2026-03-03)

**전환 사유:**
1. MediaPipe short_range 모델이 압축된 이미지에서 작은 얼굴 감지 실패
2. 다중 얼굴 감지 제한 (~2개)
3. 한국인 얼굴 인식 성능 부족
...
```
원본 이미지 → browser-image-compression (1024px 압축)
           → face-api.js 얼굴 감지
           → Canvas 모자이크 처리 (1000px)
           → base64 출력
```
...
| 파일 | 역할 |
|------|------|
| `src/hooks/useFaceDetection.ts` | 얼굴 인식 로직 |
| `src/lib/faceBlur.ts` | 모자이크/이모티콘 처리 |
| `src/components/FaceBlurModal.tsx` | UI 컴포넌트 |
| `src/components/PhotoUploader.tsx` | 썸네일 클릭 이벤트 |
| `src/pages/Index.tsx` | 모달 상태 관리 |
